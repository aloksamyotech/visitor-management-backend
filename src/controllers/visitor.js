...
export const getVisitorDetails = async (req, res, next) => {
    const getVisitorDetails = await visitorService.getVisitorDetails(req, res, next);
    res.status(statusCodes?.ok).send(getVisitorDetails);
};

export const getDetailsByNumber = async (req, res, next) => {
    const getDetailsByNumber = await visitorService.getDetailsByNumber(req, res, next);
